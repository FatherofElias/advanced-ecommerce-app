...
class CustomerTestCase(unittest.TestCase):
    def setUp(self):
        ...
        with self.app.app_context():
            db.create_all()
        # Assuming you have a method to create an admin user or any user and generate a token
        self.token = generate_token({'id': 1, 'username': 'admin', 'role': 'admin'})
    ...
    def test_get_customer(self, mock_get_customer):
        ...
        response = self.client.get('/api/customers/1', headers=self.get_headers())
    ...
    def test_update_customer(self, mock_update_customer):
        ...
        response = self.client.put('/api/customers/1', json={'name': 'John Doe Updated'}, headers=self.get_headers())
        self.assertEqual(response.status_code, 200)
        customer_data = response.get_json()
    ...
    def test_delete_customer(self, mock_delete_customer):
        response = self.client.delete('/api/customers/1', headers=self.get_headers())
        self.assertEqual(response.status_code, 200)
